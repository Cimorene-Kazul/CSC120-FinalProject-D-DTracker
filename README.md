...
AC 12                                                                                           > armor class
Initiative +1 (11)                                                                              > initative bonus with default in parentheses
...
The way combat works is all entities (players and monsters) roll initiatve by rolling 1d20 and adding their initative modifer. They then take turns doing things, in decreasing order of initiative (higher initative goes earlier). For this reason, the term 'roll initative' has the special meaning of begin combat.
...
```
InitativeTracker myEncounter = new InitativeTracker();
...
## Part 1: The Basics
The most fundamental thing our code will do when run is roll initative for all the monsters. It will then prompt each player for their rolled initative value. After this, it will go through the iniative count until it is told to stop. On each initative, if it is someone's turn, it will either prompt the player to take their turn or print the monster's stat block depending on the situation.
...
In addition, it might also be useful for monsters to have a 'lair' boolean, and if there is a 'lair' monster, print a message to do all the lair actions on initative 20. Similarly, adding a 'heal' command that functions like 'damage' but does the opposite could come in handy.

In addition to however we track the initative count a player has, player entities should store their names. Similarly, monsters should store, along with their names, a long string containing their stat block, plus their initative modifier, AC, current HP, and maximum HP. Both should have a boolean indicating if they are player-controlled.
...
We might also want to add the ability to give monsters set initative, instead of rolling. This will help for certain major encounters.

The intended classes here are: InitativeTracker, Player, Monster, Entity. Both Player and Monster are subclasses of Entity.
